...
def worker(path,st):
    ...
    print(res)
    for data in res:
        final_start_end_subtitle.append([data[0]+st,data[1]+st,data[2]])
...
if __name__ == "__main__":
    ...
    for i in range(0,4):
        ...
        os.system(f"ffmpeg -i {file_path} -ss {sHr}:{smin}:{ssec} -t {lhr}:{lmin}:{lsec} -c copy part{i}.mp4" )
        file_path="temp_part.mp4"
        video_list.append(f"part{i}.mp4")
